...
def main():
    ...
    def preprocess_squad_batch(
        ...
        targets = [answer[0] if len(answer[0]) > 0 else "" for answer in answers]
    ...
    def preprocess_validation_function(examples):
        inputs, targets = preprocess_squad_batch(examples, question_column, context_column, answer_column)
        #print('****** preprocess_validation_function targets:',targets)
    ...
    def post_processing_function(examples: datasets.Dataset, features: datasets.Dataset, outputs: EvalLoopOutput, stage="eval"):
        ...
        #predictions = [pred.strip() for pred in decoded_preds] #this may produce predictions of larger length
        ...
        for example_index, example in enumerate(examples):
            ...
            predictions.append(decoded_preds[feature_index])

        # Extracting the references (ground truths); (may need to use different format if using multipule metrics)
        #references = [[ex["answer"]["text"]] for ex in examples]
        ############# 
        '''examples = [
            {"answer_column": ["ans1", "ans2"]},
            {"answer_column": ["ans3", "ans4", "ans5"]},
            {"answer_column": ["ans6"]}
        ]
            [", ".join(ex[answer_column]) for ex in examples]:
                ["ans1, ans2", "ans3, ans4, ans5", "ans6"]
            [ex[answer_column] for ex in examples] :
                [["ans1", "ans2"], ["ans3", "ans4", "ans5"], ["ans6"]] 
        ''' 
        ...
        '''for exact answer, the right answer could be a list, 
            ["ans1", "ans2"] means they are equivalent, ["A, B"] means need to list both
            to match the required formarts:
            defalt:  ["ans1", "ans3", "ans6"] (choose the first one)
            bleu:[[["ans1"], ["ans2"]], [["ans3"], ["ans4"], ["ans5"]], [["ans6"]]] 
            rouge: [["ans1", "ans2"], ["ans3", "ans4", "ans5"], ["ans6"]]
            '''
        ###############
        references = {
    ...
    if training_args.do_eval:
        ...
        #trainer.log_metrics may have TypeError: unsupported format string passed to list.__format__ for some metric(i.e. bleu_precision is a list):
        #so we just print the metrics out and comment trainer.log_metrics  
    ...
    if training_args.do_predict:
        ...
        '''# Print first few predictions and references
        num_samples_to_display = 5
        for i, (pred, ref) in enumerate(zip(results.predictions, results.label_ids)):
